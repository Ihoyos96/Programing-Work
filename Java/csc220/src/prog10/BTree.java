...
public class BTree<K extends Comparable<K>, V> extends AbstractMap<K, V> implements Map<K, V> {
	 ...
	 * contains the key we are looking for, or if it is not there, where it
	 * belongs. If the keys are as stated in the previous example,
	...
	private Entry find(K key, List<Entry> list) {
		...
		// List entries are lists.
			...
			if (i >= 0) {
				return find(key, list.get(i)
						.getList() /* Change null to the list at i. */);
			}
		}
		return null;
	}
	...
	private void remove(K key, List<Entry> list) {
		...
		// Else:
			...
			Entry getEntry = list.get(index);
			// Get its list, call it the sublist.
			List<Entry> sublist = getEntry.getList();
			...
			getEntry.key = sublist.get(0).key;
			...
			if (sublist.size() == 1) {
				...
				if (index == list.size() - 1) {
					index--;
					getEntry = list.get(index);
					sublist = getEntry.getList();
				}
				...
				if (sublist.size() == 4) {
					...
					// EXERCISE
					// Insert it into list. What index? (Hint: to the right of
					// index i.)
